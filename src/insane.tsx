...
export type Def = z.core.$ZodTypeDef & {
  ...
  element?: z.ZodType;
  checks?: ReadonlyArray<{ _zod?: { def?: { check?: string; minimum?: unknown; maximum?: unknown } } }>;
};
...
export const resolve =
  <T,>(visit: (s: z.ZodType, d: Def) => T | undefined) =>
...
const containsWrapper = (...kinds: string[]) => (s: z.ZodType): boolean =>
...
type ValueOf<R> = R extends (p: infer P) => ReactNode ? (P extends { value: infer V } ? V : never) : never;
...
type Initialized<S, R, Sp> = SelfInitializing<R> extends true
  ? true
...
type ShapeOf<A extends readonly Part[]> = CleanShape<Merged<A>> extends z.core.$ZodShape
  ? CleanShape<Merged<A>>
  : never;
...
export function group<const A extends readonly Part[]>(...parts: A): z.ZodObject<ShapeOf<A>> {
  ...
  const spliceKeys = (record: Record<string, z.ZodType>) => {
    ...
    for (const k of keys) {
      if (dev() && /^\d+$/.test(k))
        console.warn(`[insane] numeric field name "${k}" — JS enumerates integer-like keys first, breaking authored order.`);
      if (dev() && !rendersOrDeferred(record[k]))
        console.warn(`[insane] group: "${k}" has no renderer — annotate it via field()/group()/list()/hidden().`);
      shape[k] = record[k];
    }
    return keys;
  };
  ...
  return z.object(shape).meta({ component: GroupRenderer } satisfies FieldMeta) as unknown as z.ZodObject<
    ShapeOf<A>
  >;
}
...
export function list<E extends z.ZodType>(element: E, opts: ListOpts = {}): z.ZodArray<E> {
  if (dev() && !rendersOrDeferred(element))
    console.warn(`[insane] list: element has no renderer — annotate it via field()/group()/list()/hidden().`);
  ...
  const ListRenderer = (p: NodeProps) => {
    ...
    return (
      <Wrap
        label={resolveTitle(p.schema)}
        items={items}
        add={canAdd ? () => fa.append((opts.seed ? opts.seed() : seedFor(element)) as never) : undefined}
        header={opts.header}
        footer={opts.footer}
      />
    );
