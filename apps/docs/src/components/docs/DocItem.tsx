    ...
    <div className="min-w-0 flex-1">
      ...
      {(metadata.previous || metadata.next) && (
        <div className="mt-8 flex items-center justify-between gap-6 border-t-[length:var(--rule-w)] border-line pt-4 font-mono text-[0.8rem]">
          {metadata.previous ? (
            <Link className="text-dim no-underline hover:text-pop" to={metadata.previous.permalink}>
 ...
 * `# heading`, matching upstream's synthetic-title rule) and the compiled
 * MDX content, styled by our own `.docs-content` rules (custom.css) rather
 * than theme-classic's `.markdown` + Infima. The right "Asides" rail
 * (../Asides.tsx) is no longer rendered here — it moved to src/theme/Root.tsx
 * as a page-height, sitewide persistent rail (matching `<Sidebar>`), so this
 * component is just the single content column now.
 *
