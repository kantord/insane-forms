...
import type { FieldProps, Shell, CollectionWrapper } from "../src";
...
const ListBox: CollectionWrapper = ({ label, items, add, header, footer }) => (
  <fieldset>
    ...
    {items.map((it) => (
      <div key={it.key}>
        {it.node}
        {it.remove && <button type="button" data-remove onClick={it.remove}>−</button>}
      </div>
    ))}
    {add && <button type="button" data-add onClick={add}>＋</button>}
    {footer}
  </fieldset>
);
...
const TextWidget = (p: FieldProps<string | undefined>) => (
  <input id={p.name} name={p.name} value={p.value ?? ""} onChange={(e) => p.onChange(e.target.value)} onBlur={p.onBlur} readOnly={p.readonly} />
);
...
const CheckWidget = (p: FieldProps<boolean>) => (
  <input id={p.name} name={p.name} type="checkbox" checked={p.value} onChange={(e) => p.onChange(e.target.checked)} onBlur={p.onBlur} />
);
...
const SelectWidget = (p: FieldProps<string> & { options?: readonly string[] }) => (
  <select id={p.name} name={p.name} value={p.value} onChange={(e) => p.onChange(e.target.value)} onBlur={p.onBlur}>
    {p.options?.map((o) => (
...
export const NumberField = insane.field({ schema: z.number(), widget: NumberWidget, shell: FieldShell });
export const CheckField = insane.field({ schema: z.boolean().default(false), widget: CheckWidget, shell: FieldShell });
